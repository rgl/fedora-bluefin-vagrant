...
iso_url="$(perl -ne '/default\s*=\s*\"(https:.+\.fedoraproject\.org.+)\"/ && print $1' <fedora-bluefin.pkr.hcl)"
version="$(echo "$iso_url" | perl -ne '/-(\d+.+)\.iso$/ && print $1')"
iso_checksum_url="$(dirname $iso_url)/Fedora-Everything-$version-x86_64-CHECKSUM"
